...
// the correct values. Thi s is played back with a 100 ms delay to elminate
...
public class MovementNetworkSync : MonoBehaviour
{
	...
	State[] m_LocalBufState = new State[40];
	...
	IEnumerator MonitorLocalMovement()
	{
		while (true)
		{
			...
			state.pos = _parentTransform.position;
			state.rot = _parentTransform.rotation;
			...
			// NEW BEHAVIOR: If prediction is off, move back toward last known good location.
	...
	void OnSerializeNetworkView(BitStream stream, NetworkMessageInfo info)
	{
		...
		else
		{
			...
			Quaternion rot = _parentTransform.rotation;//Quaternion.identity;
			stream.Serialize(ref pos);
			stream.Serialize(ref rot);
	...
	void Update()
	{
		...
		if (m_IsMine && m_FixError)
		{
			...
			// NEW BEHAVIOR: just lerp to the corrected position rather toward the current.
			//               The old way could "max out" and get stuck far away from the correct network position.
			//               It was also jumpier.
			
			//				 Check if we've been past our error limit for more than the max allowed time, just force to that pos then.
			
			m_PositionErrorTime += Time.deltaTime;
			if( m_PositionErrorTime > m_MaxPositionErrorTime )
			{
				_parentTransform.position = m_NewPosition;
			}
			else
			{
				_parentTransform.position = Vector3.Lerp(_parentTransform.position, m_NewPosition, Time.deltaTime * m_PosCorrectionSpeed );
			}
		// If we are not moving converge to the 100% accurate server position
		}
		else if (m_IsMine && !targetIsMoving)
		{			
			_parentTransform.position = Vector3.Lerp(m_BufferedState[0].pos, _parentTransform.position, 0.95F);
		}
		...
		else if (Network.isClient && !m_IsMine)
		{
			...
			if (m_BufferedState[0].timestamp > interpolationTime)
			{
				for (int i=0;i<m_TimestampCount;i++)
				{
					...
					if (m_BufferedState[i].timestamp <= interpolationTime || i == m_TimestampCount-1)
					{
						...
						_parentTransform.position = Vector3.Lerp(lhs.pos, rhs.pos, t);
						_parentTransform.rotation = Quaternion.Slerp(lhs.rot, rhs.rot, t);
			...
			else
			{
				...
				_parentTransform.localPosition = latest.pos;
				_parentTransform.localRotation = latest.rot;
